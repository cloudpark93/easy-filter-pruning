...
for layer_to_prune in range(0, len(layer_to_prune_original_model_conv)):
    if os.path.isdir('test_continuous_pruning/{}/conv/test_continuous_pruning_layer{}'.format(method, layer_to_prune + 1)) == False:
        os.makedirs('test_continuous_pruning/{}/conv/test_continuous_pruning_layer{}'.format(method, layer_to_prune + 1))
    ...
    for i in range(1, int(1/pruning_index_per)):
        if i == 1:
            ...
            model_pruned.save('test_continuous_pruning/{}/conv/test_continuous_pruning_layer{}/vgg16_cifar10_after_prune_{}_{}%.h5'.format(method, layer_to_prune+1, method, pruning_index_per*100))

        else:
            # load model to prune
            model_pruned = load_model('test_continuous_pruning/{}/conv/test_continuous_pruning_layer{}/vgg16_cifar10_after_prune_{}_{}%.h5'.format(method, layer_to_prune + 1, method, pruning_index_per * 100 * (i - 1)))
            ...
            model_pruned.save('test_continuous_pruning/{}/conv/test_continuous_pruning_layer{}/vgg16_cifar10_after_prune_{}_{}%.h5'.format(method, layer_to_prune+1, method, pruning_index_per*100*i))

        del model_pruned
        K.clear_session()

        # Evaluation after pruning
        model_pruned = load_model('test_continuous_pruning/{}/conv/test_continuous_pruning_layer{}/vgg16_cifar10_after_prune_{}_{}%.h5'.format(method, layer_to_prune+1, method, pruning_index_per*100*i))
        results = model_pruned.evaluate(x_test, y_test, verbose=0)
        np.savetxt('test_continuous_pruning/{}/conv/test_continuous_pruning_layer{}/vgg16_cifar10_after_prune_{}_{}%.csv'.format(method, layer_to_prune+1, method, pruning_index_per*100*i), results, delimiter=',')
...
for layer_to_prune in range(0, len(layer_to_prune_original_model_fc)):
    if os.path.isdir('test_continuous_pruning/{}/fc/test_continuous_pruning_layer{}'.format(method, layer_to_prune + 1)) == False:
        os.makedirs('test_continuous_pruning/{}/fc/test_continuous_pruning_layer{}'.format(method, layer_to_prune + 1))
    ...
    for i in range(1, int(1/pruning_index_per)):
        if i == 1:
            ...
            model_pruned.save('test_continuous_pruning/{}/fc/test_continuous_pruning_layer{}/vgg16_cifar10_after_prune_{}_{}%.h5'.format(method, layer_to_prune+1, method, pruning_index_per*100))

        else:
            # load model to prune
            model_pruned = load_model('test_continuous_pruning/{}/fc/test_continuous_pruning_layer{}/vgg16_cifar10_after_prune_{}_{}%.h5'.format(method, layer_to_prune + 1, method, pruning_index_per * 100 * (i - 1)))
            # prune & save conv layer
            model_pruned = pruning_filters_fc(pruning_index, layer_to_prune_for_continuous_pruning_fc, model_pruned, method)
            model_pruned.save('test_continuous_pruning/{}/fc/test_continuous_pruning_layer{}/vgg16_cifar10_after_prune_{}_{}%.h5'.format(method, layer_to_prune+1, method, pruning_index_per*100*i))

        del model_pruned
        K.clear_session()

        # Evaluation after pruning
        model_pruned = load_model('test_continuous_pruning/{}/fc/test_continuous_pruning_layer{}/vgg16_cifar10_after_prune_{}_{}%.h5'.format(method, layer_to_prune+1, method, pruning_index_per*100*i))
        results = model_pruned.evaluate(x_test, y_test, verbose=0)
        np.savetxt('test_continuous_pruning/{}/fc/test_continuous_pruning_layer{}/vgg16_cifar10_after_prune_{}_{}%.csv'.format(method, layer_to_prune+1, method, pruning_index_per*100*i), results, delimiter=',')
        print('Test loss for pruned model: ', results[0])
        print('Test accuracy for pruned model: ', results[1])
...
for method_count in range(1):
    conv_layers_acc = []
    conv_layers_loss = []
    file_number = sum([len(d) for r, d, files in os.walk("test_continuous_pruning/L1norm/conv/")])
    ...
    for layer in range(1, file_number+1):
        ...
        for i in range(1, int(1/(pruning_index/100))):
            data = csv.DictReader(open("test_continuous_pruning/L1norm/conv/test_continuous_pruning_layer{}/vgg16_cifar10_after_prune_{}_{}%.csv"
...
for method_count in range(1):
    fc_layers_acc = []
    fc_layers_loss = []
    file_number = sum([len(d) for r, d, files in os.walk("test_continuous_pruning/L1norm/fc/")])
    ...
    for layer in range(1, file_number+1):
        ...
        for i in range(1, int(1/(pruning_index/100))):
            data = csv.DictReader(open("test_continuous_pruning/L1norm/fc/test_continuous_pruning_layer{}/vgg16_cifar10_after_prune_{}_{}%.csv"
                                       .format(layer, file_sub_name[method_count], pruning_index * i)))
            for raw in data:
                pruned_acc.append(list(raw.values()))
                pruned_loss.append(list(raw.keys()))
